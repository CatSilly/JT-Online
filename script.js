...
process(message) {
    switch (message.type) {
        ...
        case 'player_died':
    if (message.player_id === this.client.playerId) {
        this.client.renderer.showDeathEffect();
    }
    break;
    
case 'player_respawn':
    this.client.renderer.showRespawnMessage(message.message);
    break;
...
class Renderer {
    ...
    el.style.cssText = `
        ...
        animation: deathPulse 2s ease-out forwards;
    `;
...
updateHPDisplay() {
    ...
    if (levelElement && this.playerStats.level) {
    ...
    // Update EXP
    const expFill = document.querySelector('.player-exp-fill');
